...
export async function up() {
  ...
  await query(
    `ALTER TABLE parties
       ADD COLUMN IF NOT EXISTS food_type VARCHAR(20) NULL
         CONSTRAINT chk_parties_food_type CHECK (food_type IN ('veg', 'non_veg', 'vegan')),
  ...
  await query(
    `CREATE INDEX IF NOT EXISTS idx_parties_private_code ON parties (private_code)`,
    []
  );

  // Index to efficiently exclude private parties from discovery
  await query(
    `CREATE INDEX IF NOT EXISTS idx_parties_is_private ON parties (is_private)`,
    []
  );
}
